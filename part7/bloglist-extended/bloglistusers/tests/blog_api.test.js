...
describe("when there are initially some blogs saved and two users are created", () => {
  beforeEach(async () => {
    ...
    const user = new User({
      ...
      _id: "6887a4f629d7ff494cb386a0",
      blogs: helper.initialBlogs.slice(0, 3).map((blog) => blog._id),
    });
    await user.save();
    ...
    const otherUser = new User({
      username: "Diego",
      name: "Diego",
      passwordHash: passwordHashOtherUser,
      _id: "6887a50129d7ff494cb386a2",
      blogs: helper.initialBlogs.slice(3, 6).map((blog) => blog._id),
    });
  ...
  describe("search and/or deletion of a blog", () => {
    test("succeeds when searching an existing blog", async () => {
      const response = await api.get(`/api/blogs/${helper.initialBlogs[0]._id}`).expect(200);
    ...
    test("fails when searching a blog with an invalid id", async () => {
      const response = await api.get(`/api/blogs/${await helper.nonExistingId()}`).expect(404);
  ...
  describe("update of a blog", () => {
    test("suceeds when updating an existing blog with correct data, and updates only entered fields", async () => {
      ...
      await api.put(`/api/blogs/${helper.initialBlogs[0]._id}`).send(newBlog).expect(200);
